...
<style>
  ...
  select { background:#222; color:#eee; border:1px solid #444; padding:6px; border-radius:4px; }
  .section { margin-top:32px; background:#1a1a1a; padding:20px; border-radius:8px; }
...
<div class="section">
  ...
  <form method="POST" action="/add-key">
    <div class="row">
      <input type="text" name="key" placeholder="e.g. KEY-001" required>
      <select name="days">
...
<div class="section">
  <h2>📦 Script URL</h2>
  <p style="color:#aaa;font-size:13px">Paste the raw URL of your script here. Anyone with a valid key will load this. Change it anytime without touching the Lua loader.</p>
  <form method="POST" action="/set-script">
    <div class="row">
      <input type="text" name="url" class="wide" value="${scriptUrl}" placeholder="https://pastebin.com/raw/...">
